...
import { Client } from '@proj-rin/server-sdk'
...
export interface RinAdapterConfig {
  url?: string
  token?: string
  credentials: {
    apiKey?: string
...
export class RinAdapter {
  private client: Client
  private ctx: Context
  private twitterServices: TwitterServices
  private config: RinAdapterConfig

  constructor(ctx: Context, config: RinAdapterConfig) {
    this.ctx = ctx
    this.config = config
  ...
  private setupEventHandlers(): void {
    ...
    this.client.onEvent('ui:configure', async (event) => {
      if (event.data && event.data.moduleName === 'x' && event.data.config && isXConfig(event.data.config)) {
        ...
        const credKeys: (keyof RinAdapterConfig['credentials'])[] = ['apiKey', 'apiSecret', 'accessToken', 'accessTokenSecret']
...
${tweets.map((t: Tweet) => `- ${t.author.displayName}: ${t.text.substring(0, 80)}...`).join('\n')}`,
  ...
  /**
   * Start the RinAdapter and connect to the AIRI server
   */
  ...
  /**
   * Stop the RinAdapter and disconnect from the AIRI server
   */
